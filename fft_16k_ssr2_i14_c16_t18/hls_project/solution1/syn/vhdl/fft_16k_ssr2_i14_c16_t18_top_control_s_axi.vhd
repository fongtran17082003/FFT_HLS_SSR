...
entity fft_16k_ssr2_i14_c16_t18_top_control_s_axi is
generic (
...
end entity fft_16k_ssr2_i14_c16_t18_top_control_s_axi;
...
-- 0x24 : reserved
-- 0x28 : Data signal of iter
--        bit 31~0 - iter[31:0] (Read/Write)
-- 0x2c : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of fft_16k_ssr2_i14_c16_t18_top_control_s_axi is
    ...
    constant ADDR_DIN_DATA_1  : INTEGER := 16#14#;
    constant ADDR_DIN_CTRL    : INTEGER := 16#18#;
    constant ADDR_DOUT_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_DOUT_DATA_1 : INTEGER := 16#20#;
    constant ADDR_DOUT_CTRL   : INTEGER := 16#24#;
    constant ADDR_ITER_DATA_0 : INTEGER := 16#28#;
    constant ADDR_ITER_CTRL   : INTEGER := 16#2c#;
    ...
    signal int_din             : UNSIGNED(63 downto 0) := (others => '0');
    signal int_dout            : UNSIGNED(63 downto 0) := (others => '0');
    signal int_iter            : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DIN_DATA_1 =>
                        rdata_data <= RESIZE(int_din(63 downto 32), 32);
                    ...
                    when ADDR_ITER_DATA_0 =>
                        rdata_data <= RESIZE(int_iter(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    iter                 <= STD_LOGIC_VECTOR(int_iter);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DIN_DATA_1) then
                    int_din(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_din(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ITER_DATA_0) then
                    int_iter(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_iter(31 downto 0));
                end if;
            end if;
        end if;
    end process;
